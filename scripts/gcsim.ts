...
import {
  GCSim,
  GCSimScript,
  GCSimScriptCharacterStat,
  GCSimScriptCharacterInfo,
  GCSimScriptOptions,
  GCSimScriptParam,
  GCSimScriptSetInfo,
  GCSimScriptWeaponInfo,
  GCSimScriptEnergySettings,
  GCSimScriptEnergySettings_EnergyType,
  gCSimScriptEnergySettings_EnergyTypeFromJSON,
  GCSimScriptTarget,
  GCSimScriptHurtSettings,
  gCSimScriptHurtSettings_HurtTypeFromJSON,
...
const gcsimCharRegx =
  /\s*(?<char>\w+)\s+((?<ch>char)|add\s+(?<stats>stats)|add\s+(?<ws>weapon|set)\s*=\s*\"(?<wsname>\w+)\")\s+(?<attrs>.*?)\s*;/gm;
const gcsimEnergyRegx = /\s*energy\s+(?<type>once|every)\s+(?<attrs>.*?)\s*;/gm;
const gcsimTargetRegx = /^[ \t]*target(?:[ \t]+(?<attrs>.*?))?[ \t]*;[ \t]*$/gm;
...
const gcsimTargetTypeRegx =
  /type\s*=\s*(?<typename>\w+)(?:\[(?<params>[^\]]+)\])?/;
...
const normalizeCatalogKey = (key: string): string =>
...
const gcsimCharacterToCharacter = (
  char: string,
  sourceId: string
): Character => {
  ...
  if (char.includes("lumine")) {
    ...
    const normalizedCharacter = normalizeCatalogKey(char);
    appKey = Object.keys(CHARACTERS).find(
      (key) => normalizeCatalogKey(key) === normalizedCharacter
    );
  }
...
const gcsimWeaponToWeapon = (weapon: string, sourceId: string): Weapon => {
  ...
  throw new Error(
    `${sourceId}: GCSIM weapon "${weapon}" is not available in app data`
  );
};
const gcsimSetToSet = (set: string, sourceId: string): Set => {
  ...
  throw new Error(
    `${sourceId}: GCSIM artifact set "${set}" is not available in app data`
  );
};
...
const parseStats = (line: string): GCSimScriptCharacterStat[] => {
  ...
  for (let match of line.matchAll(gcsimKeyValRegx)) {
    if (match.groups?.key && statMap.hasOwnProperty(match.groups?.key)) {
      stats.push(
        GCSimScriptCharacterStat.fromJSON({
  ...
  for (let match of script.matchAll(gcsimCharRegx)) {
    ...
    if (match.groups.ch) {
      ...
      if (!hasLevel || !hasCons || !hasTalents) {
        console.log(
          `Character ${char} missing level/constellation/talents: ${attrs}`
        );
      }
      ...
      characters[char].weaponInfo = parseWeaponInfo(
        weapon,
        match.groups.attrs,
        sourceId
      );
      ...
      characters[char].setInfos.push(
        parseSetInfo(set, match.groups.attrs, sourceId)
      );
    } else {
...
const parseEnergy = (script: string, parsedScript: GCSimScript) => {
  for (let match of script.matchAll(gcsimEnergyRegx)) {
    ...
    energy.type = gCSimScriptEnergySettings_EnergyTypeFromJSON(
      match.groups?.type.toUpperCase()
    );
    for (let attr of match.groups?.attrs.matchAll(gcsimKeyValRegx) ?? []) {
      if (attr.groups?.key === "interval") {
        const intervals = attr.groups.value
          .split(",")
  ...
  for (let match of script.matchAll(gcsimTargetRegx)) {
    ...
    for (let attr of attrs.matchAll(gcsimKeyValRegx)) {
      if (attr.groups?.key === "pos") {
        target.position = attr.groups.value
          .split(",")
        ...
        target.particleElement = elementFromJSON(
          attr.groups.value.toUpperCase()
        );
...
const parseHurt = (script: string, parsedScript: GCSimScript) => {
  for (let match of script.matchAll(gcsimHurtRegx)) {
    const hurt: GCSimScriptHurtSettings = {
      type: 0,
      start: 0,
      amount: { min: 0, max: 0 },
      element: 0,
    };
    hurt.type = gCSimScriptHurtSettings_HurtTypeFromJSON(
      match.groups?.type.toUpperCase()
    );
    for (let attr of match.groups?.attrs.matchAll(gcsimKeyValRegx) ?? []) {
      if (attr.groups?.key === "interval") {
        ...
        const amounts = attr.groups.value
          .split(",")
...
const listScriptFiles = async (directory: string): Promise<string[]> => {
  ...
  for (const entry of entries) {
    if (!entry.isFile()) {
      throw new Error(
        `unexpected entry in GCSIM script snapshot: "${entry.name}"`
      );
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  parseScripts().then(async (gcsim) => {
    ...
    await fs.promises.writeFile(
      path.join(__dirname, "../public/gcsim/gcsim.bin"),
      GCSim.encode(gcsim).finish()
    );
  });
}
